...
def ratiocalcFPLUS(dirName):
    ...
    for root, dirs , files in os.walk(dirName):
        for file in files:
            if file.endswith("LargeDS_FPlus m_q=0.40.txt"):
                print("Reading in file " + os.path.join(root, file))
                with open(os.path.join(root, file)) as f:
                    reader = csv.reader(f, delimiter = '\t')
                    for row in reader:
    ...
    csv_writer(dirName + "\FPlus_over_Gauge_mq=0.40.txt", mut_gsquared, m_ratio)
...
def ratiocalcGPLUS(dirName):
    ...
    for root, dirs , files in os.walk(dirName):
        for file in files:
            if file.endswith("LargeDS_GPlus m_q=0.40.txt"):
                print("Reading in file " + os.path.join(root, file))
                with open(os.path.join(root, file)) as f:
                    reader = csv.reader(f, delimiter = '\t')
                    for row in reader:
    ...
    csv_writer(dirName + "\GPlus_over_Gauge_mq=0.40.txt", mut_gsquared, m_ratio)
